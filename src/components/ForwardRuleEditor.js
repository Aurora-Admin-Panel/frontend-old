...
const ForwardRuleEditor = ({
  ...
  useEffect(() => {
    if (forwardRule) {
      ...
      if (forwardRule.config.ServeNodes) setServeNodes(forwardRule.config.ServeNodes);
      if (forwardRule.config.ChainNodes) setChainNodes(forwardRule.config.ChainNodes);
    }
  }, [forwardRule]);
              ...
              <>
                <Label className="mt-4 flex flex-row justify-between items-center items-center">
                    ...
                    <div className="flex flex-3 items-center">
                      <button className="mr-3 h-5 w-5 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-500 border border-transparent rounded active:bg-red-600 hover:bg-red-700 focus:outline-none focus:shadow-outline-red"
                        onClick={() => retries > 0 ? setRetries(retries-1) : setRetries(0)}>
                        <MinusIcon />
                      </button>
                      <span className="w-3 h-6 text-sm text-black dark:text-gray-300">{retries}</span>
                      <button className="ml-3 h-5 w-5 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-green-400 border border-transparent rounded active:bg-green-600 hover:bg-green-700 focus:outline-none focus:shadow-outline-green"
                        onClick={() => retries < 5 ? setRetries(retries+1) : setRetries(5)}
                      >
                        <PlusIcon />
                      </button>
                  </div>
